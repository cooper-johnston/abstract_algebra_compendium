...
\begin{enumerate}
    ...
    \item there exists a \defnem{neutral element} $ e\in G $ such that for every $ g\in G $, we have $ g\odot e=e\odot g=g $; and
    \item for every $ g\in G $, there exists an \defnem{inverse element} $ g^{-1}\in G $ such that $ g\odot g^{-1}=g^{-1}\odot g=e $.
\end{enumerate}
A group whose law is commutative is called \defnem{abelian}.
\end{defn}
...
\begin{defn}
Let $ x $ be some element in a group. Then, the cardinality of $ \langle x\rangle $ is called the \defnem{order} of $ x $, denoted $ \ord(x) $.
\end{defn}
...
\begin{cor}[Alternative definition of the left and right cosets]\label{cor:cosets_defn}
...
\begin{thm}\label{thm:cosets_group_structure}
